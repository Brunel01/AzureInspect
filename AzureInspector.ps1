<#
    .SYNOPSIS
    Automated data collection to help in the security assessment of Microsoft Azure environments.

    .DESCRIPTION
    Automated data collection to help in the security assessment of Microsoft Azure environments and develop a risk-based remedation plan with step-by-step remediation guidance and best practice recommendations.

    .PARAMETER OutPath
    The path to a folder where the report generated by main script will be placed.

    .PARAMETER Auth
    REQUIRED - Switch that should be one of the literal values "MFA", "DEVICE", "APP", or "ALREADY_AUTHED". Auth controls how the main script will authenticate to the necessary services.
    ...
    The name or names of the inspector or inspectors needed to run with the main script. If multiple inspectors are selected they must be comma separated. Only the named inspectors will be run.

    .PARAMETER ExcludedInspectors
    The name or names of the inspector or inspectors you wish to prevent from running with the main script. If multiple inspectors are selected they must be comma separated. All modules other included modules will be run.

    .INPUTS
    None. You cannot pipe objects to main script

    .OUTPUTS
    None. The main script does not generate any output.

    .EXAMPLE
    PS> .\MainScript.ps1 -OrgName mycompany -OutPath ..\Azure_report -Auth MFA

    .EXAMPLE
    PS> .\MainScript.ps1 -OrgName mycompany -OutPath ..\Azure_report -Auth DEVICE

    .EXAMPLE
    PS> .\MainScript.ps1 -OrgName mycompany -OutPath ..\Azure_report -Auth MFA -SelectedInspectors inspector1, inspector2

    .EXAMPLE
    PS> .\MainScript.ps1 -OrgName mycompany -OutPath ..\Azure_report -Auth MFA -ExcludedInspectors inspector1, inspector2
#>
...
param (
    ...
    #===========================Parameters for Azure Storage===========================
    [Parameter(Mandatory = $true,
        HelpMessage = 'Azure Storage Account Name')]
    [string] $StorageAccountName,
    [Parameter(Mandatory = $true,
        HelpMessage = 'Azure Storage Account Key')]
    [string] $StorageAccountKey,
    [Parameter(Mandatory = $true,
        HelpMessage = 'Azure Storage Container Name')]
...
Function Create-JsonReport {
    ...
    $jsonFilePath = "$OutputPath\$OrgName`_Report.json"
    $report | ConvertTo-Json -Depth 4 | Set-Content -Path $jsonFilePath
...
Foreach ($subscription in $subscriptions) {
    ...
    Try {
        ...
        function Parse-Template {
            $template = (Get-Content ".\DefaultTemplate.html") -join "`n"
        ...
        $flags = "<b>Prepared for:</b><br/>" + $org_name + "<br/><br/>"
        ...
        # Create JSON report
        $jsonFilePath = Create-JsonReport -Findings $findings -OutputPath $out_path -OrgName $org_name -AuthType $Auth -SubscriptionName $subscription.Name -SubscriptionId $subscription.Id -SelectedInspectors $selected_inspectors
        Write-Output "JSON report created successfully at $jsonFilePath."
    }
    Catch {
        Write-Warning "Error message: $_"
...
Function JSON-Report {
    ...
    foreach ($finding in $sortedFindings) {
        If ($null -NE $finding.AffectedObjects) {
            ...
            $result = [PSCustomObject]@{
                ...
                Inspectors = $inspectors.Count.ToString()
                FindingName     = $finding.FindingName
    ...
    $results | ConvertTo-Json | Out-File -FilePath $out_path\TestReport_$(Get-Date -Format "yyyy-MM-dd_hh-mm-ss").json
}
#========================================Fin====================================================
...
Function CSV-Report {
    ...
    $results | Export-Csv "$out_path\TestReport_$(Get-Date -Format "yyyy-MM-dd_hh-mm-ss").csv" -Delimiter '^' -NoTypeInformation -Append -Force
...
$compress = @{
    ...
    DestinationPath  = "$out_path\$($org_name)_TestReport.zip"
}
...
#================================Compress and Upload the Report#================================
# After generating and saving the report
Try {
    $compress = @{
        Path             = $out_path
        CompressionLevel = "Fastest"
    ...
    Compress-Archive @compress
    Write-Output "Report compressed successfully."

    # Upload the compressed report to Azure Storage
    Upload-ToAzureStorage -FilePath "$out_path\$($org_name)_TestReport.zip" -ContainerName $StorageContainerName -StorageAccountName $StorageAccountName -StorageAccountKey $StorageAccountKey
    Write-Output "Report uploaded to Azure Storage successfully."
}
Catch {
    Write-Warning "Failed to compress or upload the report: $_"
    # Add error logging here if needed
}
#================================Fin#================================


#====================Generate and Upload the JSON Report#====================
...
Try {
    ...
    $compress = @{
        Path             = $out_path
        CompressionLevel = "Fastest"
        DestinationPath  = "$out_path\$($org_name)_JSon_Report.zip"
    }
    Compress-Archive @compress
    Write-Output "Report compressed successfully."

    # Upload the compressed report to Azure Storage
    Upload-ToAzureStorage -FilePath "$out_path\$($org_name)_JSon_Report.zip" -ContainerName $StorageContainerName -StorageAccountName $StorageAccountName -StorageAccountKey $StorageAccountKey
    Write-Output "Compressed report uploaded to Azure Storage successfully."

    # Upload the JSON report to Azure Storage
    Upload-ToAzureStorage -FilePath $jsonFilePath -ContainerName $StorageContainerName -StorageAccountName $StorageAccountName -StorageAccountKey $StorageAccountKey
    Write-Output "JSON report uploaded to Azure Storage successfully."
}
Catch {
    Write-Warning "Failed to compress or upload the reports: $_"
    # Add error logging here if needed
}
#=====================================End=======================================
